"""Mutable user aggregate and sticker-tag behavior.

The aggregate owns the private/public pack decision, ordered sticker ids, and
the read cache used by inline queries. It deliberately knows nothing about
Telegram updates or persistence; repositories serialize its current state.
"""
...
class StickfixUser:
  """A regular sticker owner or the shared public pack.

  ``stickers`` and ``cached_stickers`` map each tag to an ordered list of
  sticker ids. Mutations happen in place before a repository persists the
  aggregate. Regular users carry a numeric ``UserId``; the shared pack uses
  the ``SF_PUBLIC`` sentinel.

  When ``private_mode`` is false, reads and writes may target the shared pack.
  Cache entries belong to the regular user because they combine private and
  public results for that caller.
  """
